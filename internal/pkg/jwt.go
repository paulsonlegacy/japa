...
func GenerateJWT(user *entity.User) (string, error) {
    ...
    claims := jwt.MapClaims{
        ...
        "exp":      time.Now().Add(config.Settings.JWT.Expiry).Unix(), // Expiration time as UNIX timestamp
        "iss":      config.Settings.JWT.Issuer,  // Issuer identifier
    }
    ...
    signedToken, err := token.SignedString([]byte(encodeBase64(config.Settings.JWT.JWTSecretKey)))
...
func ValidateJWT(tokenString string) (jwt.MapClaims, error) {
    ...
    token, err := jwt.ParseWithClaims(
        ...
        func(token *jwt.Token) (interface{}, error) {
            ...
            return []byte(encodeBase64(config.Settings.JWT.JWTSecretKey)), nil
        },
    )
    if err != nil {
